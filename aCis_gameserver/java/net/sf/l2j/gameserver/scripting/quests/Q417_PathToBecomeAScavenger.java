...
public class Q417_PathToBecomeAScavenger extends Quest
{
	...
	public String onKill(Npc npc, Creature killer)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case HONEY_BEAR:
				if (st.getInt("cond") == 5 && ((Monster) npc).getSpoilState().isActualSpoiler(player) && st.dropItemsAlways(HONEY_JAR, 1, 5))
					st.set("cond", "6");
				break;
			
			case HUNTER_TARANTULA:
			case PLUNDER_TARANTULA:
				if (st.getInt("cond") == 7 && ((Monster) npc).getSpoilState().isActualSpoiler(player) && st.dropItems(BEAD, 1, 20, (npc.getNpcId() == HUNTER_TARANTULA) ? 333333 : 600000))
					st.set("cond", "8");
				break;
		}
