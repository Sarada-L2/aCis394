	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("30103-06.htm"))
		{
			if (st.getQuestItemsCount(57) >= 2000)
			{
				...
				if (giveDimensionalDiamonds35(st))
		...
		switch (npc.getNpcId())
		{
			...
			case ANT_CAPTAIN:
				if (st.hasQuestItems(PINTER_INSTRUCTIONS))
				{
					...
					if (st.dropItemsAlways(AMBER_BEAD, (player.getClassId() == ClassId.SCAVENGER && ((Monster) npc).getSpoilState().isActualSpoiler(player)) ? 10 : 5, 70))
