...
public final class GeoDataConverter
{
		...
		try
		{
			for (int x = 0; x < GeoStructure.REGION_CELLS_X; x++)
			{
				for (int y = 0; y < GeoStructure.REGION_CELLS_Y; y++)
				{
					...
					int index;
					while ((index = block.getIndexBelow(x, y, height, null)) >= 0)
					{
						// get height and nswe
						height = block.getHeight(index, null);
	...
	private static final byte getNsweBelow(int geoX, int geoY, short worldZ)
	{
		...
		final int index = block.getIndexBelow(geoX, geoY, worldZ, null);
		return index < 0 ? 0 : block.getNswe(index, null);
	}
